/**************************************************************************
 Program:  Expiration_rpt_5_yrs.sas
 Library:  PresCat
...
ods tagsets.excelxp file="&_dcdata_default_path\PresCat\Prog\Reports\Expiration_rpt_5_yrs_&rpt_suffix..xls" 
  style=Normal 
...
ods tagsets.excelxp options(sheet_name="Project-Based");

title1 "DC Preservation Catalog: Project-Based Subsidies Expiring Within 5 Years";
...
proc report data=Expiration_rpt nowd
  ...
  where (intnx( 'day', today(), 0, 'same' )) <= poa_end <= (intnx( 'year', today(), 5, 'same' )) and 
    subsidy_active and portfolio ~= "LIHTC";
  column nlihc_id poa_end rpt_id program units_assist;
  define nlihc_id / "Catalog ID" display;
  define rpt_id / "Project" display;
  define poa_end / display;
  define program / display;
  define units_assist / display;
  label category_code = 'Category';
run;

ods tagsets.excelxp options(sheet_name="LIHTC compliance");

title1 "DC Preservation Catalog: LIHTC Compliance Period Ending Within 5 Years";
footnote2 height=9pt "Tax exempt bonds excluded. LIHTC end date may be approximate.";

proc report data=Expiration_rpt nowd
      style(header)=[fontsize=2 font_weight=bold textalign=left]
      style(column)=[fontsize=2 textalign=left];
  where (intnx( 'year', today(), 0, 'same' )) <= compl_end <= (intnx( 'year', today(), 5, 'same' )) and 
    subsidy_active and portfolio = "LIHTC";
  column nlihc_id compl_end rpt_id program units_assist;
  define nlihc_id / "Catalog ID" display;
  define rpt_id / "Project" display;
...
title1 "DC Preservation Catalog: LIHTC Extended Use Period Ending Within 5 Years";
footnote2 height=9pt "Tax exempt bonds excluded. LIHTC end date may be approximate.";

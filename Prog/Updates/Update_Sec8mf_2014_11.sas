...
data Subsidy_mfa_update_a;
  ...
  update 
    Subsidy_mfa Sec8MF_subsidy_update (keep=&Subsidy_update_vars &Subsidy_missing_info_vars);
  by Subsidy_Info_Source_ID;
...
data Subsidy_mfa_except;
  ...
  update Subsidy_mfa_update_b (in=in1) Subsidy_except_test (drop=Except_:);
  by nlihc_id Subsidy_ID;
...
** Archive past Catalog **;

/** Macro Archive_catalog_data - Start Definition **/

%macro Archive_catalog_data( data=, zip_suf=, zip_pre=, path=&_dcdata_r_path\PresCat\Data );
  ...
  %do %until ( &v = );
    ...
    x "&_dcdata_r_drive:\Tools\7-zip\7z a -tzip &path\Archive\&zip_pre._&zip_suf &path\&v..sas7bdat";
...
/****DISABLE****%Archive_catalog_data( data=Project Subsidy, zip_suf=Update_&Update_file )*****/
...
ods listing close;
ods pdf file="L:\Libraries\PresCat\Prog\Updates\Update_Subsidy_Sec8mf_2014_11.pdf" 
